...
function handleSlide(e) {
  ...
  if (
    ...
    if (canMoveLeft() && (e.code == "ArrowLeft" || e.code == "KeyA")) {
      slideLeft();
      setTwo();
    } else if (canMoveRight() && (e.code == "ArrowRight" || e.code == "KeyD")) {
      slideRight();
      setTwo();
    } else if (canMoveUp() && (e.code == "ArrowUp" || e.code == "KeyW")) {
      slideUp();
      setTwo();
    } else if (canMoveDown() && (e.code == "ArrowDown" || e.code == "KeyS")) {
      slideDown();
      setTwo();
    }
...
function slideLeft() {
  ...
  for (let r = 0; r < rows; r++) {
    let row = board[r];
    let originalRow = row.slice(); // store the origin values of row array
  ...
  for (let r = 0; r < rows; r++) {
    for (let c = 0; c < columns; c++) {
      if (board[r][c] == 2048 && is2048Exist == false) {
        alertMessage("You win! You got the 2048!");
        is2048Exist = true;
      } else if (board[r][c] == 4096 && is4096Exist == false) {
        alertMessage(
          "You are unstoppable at 4096! You are fanstastically unstopable!"
        );
        is4096Exist = true;
      } else if (board[r][c] == 8192 && is8192Exist == false) {
        alertMessage(
          "Victory! You have reached 8192! You are incredibly awesome!"
        );
        is8192Exist = true;
      }
    }
...
// This will listen to when we touch screen and assigns the x coordinate of that touch.
...
document.addEventListener('touchmove', (e) => {
  if(!e.target.className.inclues("tile")) return;
